...
public class ChuiXue_Anime implements PageProcessor{
	//白兔糖
	private String url = "http://www.chuiyao.com/manhua/21165/";
	...
	private Site site = Site.me()
			...
			.addStartUrl(url);
	public static final String LIST_All = "^http://www\\.chuiyao\\.com/\\w+/\\d+/$";
	public static final String ARTICLE  = "http://www\\.chuiyao\\.com/\\w+/\\d+/\\d+\\.html";
	...
	public void process(Page page) {
		if(page.getUrl().regex(LIST_All).match()){
			...
			String name = page.getHtml().xpath("//div[@class='topnav_l']/span[2]/a/text()").toString();
			System.out.println("名称:"+name);
			String chapter = page.getHtml().xpath("//div[@class='topnav_l']/h1/text()").toString();
			...
			String regExp = "var qTcms_S_m_murl_e = \".*\\\";";
			Pattern pattern = Pattern.compile(regExp);
			Matcher matcher = pattern.matcher(page.getHtml().toString());
			boolean findRet = matcher.find();
			if(findRet){
				String all = matcher.group(0).replace("\";var qTcms_S_m_murl_e2=", "").replace("var qTcms_S_m_murl_e = \"", "").replace("\";", "");
